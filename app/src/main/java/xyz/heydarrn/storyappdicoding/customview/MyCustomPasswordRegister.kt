...
class MyCustomPasswordRegister : AppCompatEditText, View.OnTouchListener {
    private lateinit var clearPasswordText:Drawable

    constructor(context: Context) : super(context) { initializeCustomPassword()}
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) { initializeCustomPassword()}
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
        context,
        attrs,
        defStyleAttr
    ) { initializeCustomPassword()}

    private fun initializeCustomPassword() {
        ...
        addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(p0: Editable?) {
                if (text?.length in 1..5) {
                    error=resources.getString(R.string.password_not_long_enough)
                }
            }
    ...
    override fun onTouch(p0: View?, p1: MotionEvent?): Boolean {
        if (compoundDrawables[2] != null) {
            ...
            if (layoutDirection == View.LAYOUT_DIRECTION_RTL) {
                clearButtonEnd = (clearPasswordText.intrinsicWidth + paddingStart).toFloat()
                when {
                    p1!!.x < clearButtonEnd -> isClearButtonClicked = true
                }
            } else {
                clearButtonStart = (width - paddingEnd - clearPasswordText.intrinsicWidth).toFloat()
                when {
    ...
    override fun onDraw(canvas: Canvas?) {
        super.onDraw(canvas)
        hint=resources.getString(R.string.password_hint_register)
